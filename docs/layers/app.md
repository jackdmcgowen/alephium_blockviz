...
| Style tokens | `style_blockflow.hpp` + JSON — `walk_trace` ≠ gold; hop/sobel fade; `block_pop_*` admit scale |
| Motion easing | `motion_easing.hpp` — ease-out-back pop-in on first visible-ring admit (N-cap) |
