...
void SobelAsyncPass::submit(frame_graph::OutlinePass& outline,
                            ...
                            uint32_t image_index)
{
    ...
    if (!ctx.device || !ctx.graphics_queue || !ctx.compute_queue || !ctx.main_pass ||
        !ctx.frame_sync || !ctx.presenter || !ctx.outline_instance_buffer)
        throw std::runtime_error("async sobel: incomplete context");
    if (!outline.ready() || !compute.ready() || !overlay.ready() || !resources.ready())
        throw std::runtime_error("async sobel: passes not ready");
    if (done_fence_ == VK_NULL_HANDLE)
        create(ctx.device);
    ...
    const VkSemaphore g2c = resources.graphics_to_compute(frame_index);
    const VkSemaphore cfin = resources.compute_finished(frame_index);
    ...
    // Single outline pass: all cubes, one clear, colors from compact instance buffer.
    {
        ...
        frame_graph::PassRecordParams sp{};
        sp.base.cmd = graphics_cb;
        sp.frame_ubo_set = ctx.frame_ubo_set;
        ...
        outline.record(sp);
        outline.record_sel_depth_release_for_compute(graphics_cb);
        outline.record_outline_color_to_shader_read(graphics_cb);
    }
    ctx.main_pass->end_command_buffer(graphics_cb);
    ...
    // CMP: depth Sobel → white edge mask.
    {
        ...
        vkBeginCommandBuffer(compute_cb, &bi);
        {
            ...
            frame_graph::PassRecordParams cp{};
            cp.base.cmd = compute_cb;
            compute.record(cp);
        }
    ...
    // Overlay: edge (white) × outline color → present.
    {
        ...
        vkBeginCommandBuffer(overlay_cb, &bi);
        {
            ...
            frame_graph::PassRecordParams op{};
            op.base.cmd = overlay_cb;
            op.color_view = ctx.swapchain_color_view;
            op.width = ctx.width;
            op.height = ctx.height;
            overlay.record(op);

            if (ctx.before_present)
                ctx.before_present(ctx.before_present_user, overlay_cb, ctx.swapchain_image);
            else
                ctx.main_pass->transition_color_to_present(overlay_cb, ctx.swapchain_image);
        }
