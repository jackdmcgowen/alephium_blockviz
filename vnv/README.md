...
./scripts/run_vnv.sh --mod           # → vnv/reports/mod/<run_id>/ (index.html + artifacts.zip)
./scripts/run_vnv.sh --report-only --suite mod
```

HTML reports (pass/fail, catalog, expected/actual, before/after, artifact zips):  
uniform layout under **[reports/](reports/README.md)** (`mod` / `int` / `bench` / `bench/matrix`).  
Generator: `scripts/generate_vnv_report.py`; catalog `manifest/case_catalog.json`.
