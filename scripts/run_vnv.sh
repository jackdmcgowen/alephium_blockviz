...
#   ./scripts/run_vnv.sh --bench         # opt-in; needs GPU/window
#   ./scripts/run_vnv.sh --skip-build
#   ./scripts/run_vnv.sh --config Release
#   ./scripts/run_vnv.sh --ctest         # mod via ctest
#
# Notes:
# - int golden compare needs DISPLAY + real GPU screenshot (Linux capture is
#   stub-quality until swapchain readback; expect compare fail on CI without GPU).
# - Free GitHub CI runs mod + compile GPU harnesses only.
set -euo pipefail
...
if [[ "$RUN_INT" -eq 1 ]]; then
  ...
  echo ""
  echo "== VnV [int] int_visual (${case}) =="
  if [[ -z "$path" ]]; then
    echo "FAIL: missing int_visual (build with -DBLOCKVIZ_BUILD_VNV_GPU=ON)"
    failures=$((failures + 1))
  elif [[ -z "${DISPLAY:-}${WAYLAND_DISPLAY:-}" ]]; then
    echo "FAIL: no DISPLAY/WAYLAND_DISPLAY — cannot run GPU int harness"
    failures=$((failures + 1))
  else
    mkdir -p "$out_dir"
    rm -f "$actual"
    if ! "$path" --case "$case" --out "$actual"; then
      ...
      if python3 vnv/int/tests/visual/compare_images.py \
        ...
        echo "note: Linux screenshots may be stub/driver-divergent; use --update-goldens carefully"
        failures=$((failures + 1))
      fi
    fi
  fi
...
if [[ "$RUN_BENCH" -eq 1 ]]; then
  ...
  echo ""
  echo "== VnV [bench] bench_frame_profiler (${case}) =="
  if [[ -z "$path" ]]; then
    echo "FAIL: missing bench_frame_profiler"
    failures=$((failures + 1))
  elif [[ -z "${DISPLAY:-}${WAYLAND_DISPLAY:-}" ]]; then
    echo "FAIL: no DISPLAY/WAYLAND_DISPLAY — cannot run bench harness"
    failures=$((failures + 1))
  else
    mkdir -p "$out_dir"
    if ! "$path" --case "$case" --out "$actual"; then
